...
当前项目优先采用 `LLM WIKI.md` 所定义的知识库维护方式：不是只在提问时检索 raw 资料，而是持续把新资料和有价值的问答成果编译进可复用、可交叉引用、可演进的 wiki。

### 三层架构

- **raw 原始资料层**：文章、网页剪藏、PDF、Word、图片和数据文件的事实来源。原则上只归档、读取和补充附件，不改写原始内容。
- **wiki 知识沉淀层**：由 LLM 维护的来源页、实体页、概念页、对比页、综合页和模块总览。负责提炼结论、关联页面、记录差异、更新过时内容和形成综合判断。
- **项目模式层**：本 `CLAUDE.md`、模板、索引和日志。用于约束目录结构、页面格式、整理流程和验证方法。

### 默认整理流程

当用户要求“整理知识”“新增资料”“把内容放进知识库”或类似操作时，默认按以下顺序执行：

1. 先读取 `wiki/index.md`、`wiki/overview.md`、`wiki/shared/raw资料总清单.md`，再读取相关模块首页和目标 raw 资料。
2. 判断新资料涉及哪些既有来源页、实体页、概念页、对比页或综合页，不把一份资料孤立地整理成单页。
3. 先建立或更新来源页，再同步相关实体、概念、索引和模块入口；发现冲突时记录原结论、差异和综合判断，不直接无依据覆盖。
4. 对有价值且可复用的问答、比较和分析，在用户明确要求或内容具有长期复用价值时，归档为 wiki 页面，而不是只留在聊天记录中。
5. 每次整理都追加 `wiki/log.md` 记录，说明来源、改动范围、结论变化和后续待办。
6. 完成后检查 wiki-link、图片嵌入、frontmatter、统计数量、孤立页面和过时引用；不能确认的内容明确标注“待确认”或“资料存在不一致”。
7. 如果整理对象位于 `raw/Clippings/`，整理完成并确认目标模块后，将该文件移动到 `raw/<module>/<type>/<YYYYMM>/` 对应目录，不保留 Clippings 下的重复副本；同步修正来源页路径、raw 资料清单、统计和整理日志。若模块、类型或年月无法确定，先标记待确认，不擅自移动。

### 查询与维护原则

- 回答业务问题时，优先查询已经编译好的 wiki；只有 wiki 没有沉淀或存在明显空白时，才回到 raw 或通用知识补充，并说明依据范围。
- 新资料应当增量更新已有知识，而不是每次重写整个 wiki；避免制造重复页面和超大的综合页面。
- 定期关注页面之间的矛盾、陈旧结论、孤立页面、缺失反向链接和重要但尚未独立建页的概念。
- `wiki/index.md` 是面向内容的导航目录；`wiki/log.md` 是追加式时间线；两者用途不同，都应在整理后按需维护。
- 图片、截图和附件应尽量本地化到 `raw/assets/`，在 wiki 中使用 Obsidian `![[...]]` 嵌入；含账号、密码、token 或其他敏感信息的图片不直接嵌入 wiki。
