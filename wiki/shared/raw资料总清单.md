...
| --- | ---: | --- | --- |
| `raw/fico/` | 56 | 已完成本轮整理；新增凭证类型分析和 ZFIR020 报表使用手册；其中 1 份 FS15/FS16 Markdown 资料近重复，暂保留未重复整理 | [[wiki/fico/sources/raw资料清单]] |
| `raw/accounting/` | 5 | 两份 2022 年教材原始 PDF 均已移除，但来源页保留历史知识内容；其余资料已归档，网络教育/实务参考标记 `unknown`；既有财政部令第33号双 raw 形态仍只计 1 个规范主题 | [[wiki/accounting/sources/raw资料清单]] |
...
| 合计 | 80 个物理文件 | 当前模块化 raw 有效原始资料已完成本轮整理；两份 2022 年教材原始 PDF 已移除但来源页保留；accounting 的财政部令第33号保留网页摘录与官网复制全文两种 raw 形态但只计 1 个规范主题；FICO 重复资料和网络文章待核验边界均已标注 | 见各模块清单 |
...
`raw/fico/` 当前物理有效原始资料共 56 个，已整理到 `wiki/fico/`：

- Markdown：29 个物理文件，其中 1 份 FS15/FS16 资料近重复；新增 2 篇 202608 资料（凭证类型分析、ZFIR020 报表手册）已建立来源页
- Word：19 个
- PDF：7 个
- Excel：1 个
...
截至 2026-08-13，`raw/` 下按当前物理文件口径的模块化有效原始资料为 78 个；其中 FICO 54、accounting 5、ABAP 1、SD 17、shared 1。`raw/Clippings/` 当前无有效资料文件，18 张流程图作为 shared 测试文档附件归档于 `raw/assets/`。
