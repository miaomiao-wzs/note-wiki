...
本来源页说明 `OKB9` 的业务功能，不把生产环境临时开放客户端设置当作来源文章的标准结论。用户提供的现场处理方法是：当生产客户端设置为“**不允许更改**”而无法编辑时，经授权和变更审批后，通过 `SCC4` 临时改为“**不带自动记录的更改**”，完成必要的 `OKB9`/`OKEW` 修改与验证后立即恢复为“**不允许更改**”。

该方法可能无法通过正常传输链路完整追踪，执行前应准备备份、回退、审计留痕和维护窗口，不建议作为常规生产配置策略。完整步骤见 [[wiki/fico/synthesis/生产环境 OKB9 OKEW 临时维护方法]]；`SCC4` 和 `OKEW` 的具体行为仍需按目标系统版本核验。
